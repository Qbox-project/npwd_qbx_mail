...
lib.callback.register("npwd:qb-mail:getMail", function(source)
...
lib.callback.register('npwd:qb-mail:updateRead', function(source, data)
	local src = source
...
lib.callback.register('npwd:qb-mail:deleteMail', function(source, data)
    local player = exports.qbx_core:GetPlayer(source)
	return MySQL.query.await('DELETE FROM player_mails WHERE mailid = ? AND citizenid = ?', {data, player.PlayerData.citizenid})
end)

lib.callback.register('npwd:qb-mail:updateButton', function(source, id)
    local player = exports.qbx_core:GetPlayer(source)
...
RegisterNetEvent('qb-phone:server:sendNewMail', function(mailData)
	...
	TriggerClientEvent('npwd:qb-mail:newMail', source, newMail)
end)
